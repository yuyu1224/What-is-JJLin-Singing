...
export default {
  ...
  methods: {
    createChart() {
      ...
      this.data.sort((a, b) =>
        d3.ascending(parseDate(a.date), parseDate(b.date)),
      );
      const colorScale = d3
        .scaleOrdinal()
        .domain([
          d3.min(this.data, (d) => d.score),
          d3.max(this.data, (d) => d.score),
        ])
        .range([
          "#FF5C00",
          "#96753C",
      ...
      const x = d3
        ...
        .domain([
          new Date(
            d3.min(validData, (d) => parseDate(d.date)).getFullYear() - 1,
            0,
            1,
          ), // 最小日期 -1年
          new Date(
            d3.max(validData, (d) => parseDate(d.date)).getFullYear() + 1,
            11,
            31,
          ), // 最大日期 +1年
        ])
      ...
      this.showTooltip = function (event, d) {
        ...
        tooltip
          .html(
            `专辑名称: ${d.album}<br>发行时间: ${d.date}<br>评分人数: ${
              d.evaluations
            }<br>评分: ${d.score.toFixed(1)}`,
          )
